...
def game():
    global h_x, h_y, e_x, e_y, e2_x, e2_y, torpeda_x, torpeda_y, torpeda_xs, torpeda_ys, torpeda_flag, e_boom, e2_boom, e_boom_time, e2_boom_time, boom_max_time, h_score, h_score_next_level, h_hp, state    
    if h_hp <=0:
        state = 3
        print("sdfsdf")
    #Действия
    
    #Если персонаж вылез за границы экрана, то возвращаем его обратно
    #if b_left == True:
    #    h_x -= h_xs
    #    if h_x <=0:
    #        h_x = 0
    #if b_right == True:
    #    h_x += h_xs
    #    if h_x + h_img.get_width() >= 800:
    #        h_x = 800 - h_img.get_width() 
    if b_left == True:
        h_x -= h_xs
